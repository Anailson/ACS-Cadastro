...
class AgentsPersistence
{
    ...
    {
        $db = new AcsDataBase(AcsDataBase::DB_NAME);
        $query = "SELECT A.ID, A.NAME, A.NUM_SUS, A.AREA, A.EQUIP FROM TB_AGENT AS A WHERE A.NUM_SUS = :NUM_SUS";
    ...
    {
        $db = new AcsDataBase(AcsDataBase::DB_NAME);
        $query = "INSERT INTO " . self::TB_AGENT . "(" . self::NAME . "," . self::NUM_SUS . "," . self::AREA . "," . self::EQUIP . ")
